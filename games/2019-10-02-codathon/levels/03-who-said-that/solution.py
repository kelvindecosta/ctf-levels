...
if __name__ == "__main__":
    ...
    *   The recipient verifies the message with the signature and the sender's public key
    ...
    for sender, recipient in itertools.permutations(people, 2):
        snd_pub_key, _ = load_keys(path.join("people", sender))
        _, rcp_prv_key = load_keys(path.join("people", recipient))

        try:
            flag = decrypt(cphr_txt, rcp_prv_key)
            if verify(flag, signature, snd_pub_key):
